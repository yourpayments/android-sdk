...
suspend fun payWithCard(container: ViewGroup) {
    ...
    val intent = YP.createIntent(
        CreateIntentRequest(
            ...
            amount = 100_000L,        // 1000.00 ₽ — сумма в копейках
            currency = "RUB",
            messageScheme = "SMS",
...
Адрес API задаётся в рантайме через `YpConfig` — в отличие от Web SDK, отдельных сборок под окружения нет. `merchantCode` вам выдаёт менеджер при подключении.
...
| `amount` | `Long` | Сумма в **минорных единицах** (копейки/центы). Минимум `100` |
...
| `culture` | `String?` | Локаль: `"ru-RU"` или `"en-US"` (влияет на язык страниц платёжных систем) |
...
| `orderTimeout` | `Int?` | Время жизни интента, секунды (1200–2592000). По истечении — статус `Expired` |
...
| `restrictedPaymentMethods` | `List<String>?` | Запрещённые методы: `"Card"`, `"FasterPayments"`, `"SberPay"`, `"AlfaPay"`, `"MirPay"`, `"TPay"`, `"BNPL"` |
| `items` | `List<ProductItem>?` | Состав заказа ([формат](#productitem)) |
| `receipts` | `JsonObject?` | Данные чеков (свободный формат, по согласованию) |
| `userInfo` | `UserInfo?` | Данные плательщика ([формат](#userinfo)) |
| `metaData` | `JsonObject?` | Произвольные метаданные — вернутся в интенте |
...
> **Рекомендация:** при оплате картой всегда передавайте `userInfo.billing` (достаточно `email` и/или имени). В текущей версии API оплата картой по интенту, созданному без billing-данных, может быть отклонена сервером.
...
| `getStatusDetails()` | `IntentStatusResponse` | Статус + список транзакций (для собственного поллинга) |
...
Список методов, доступных покупателю, приходит в `intent.data.paymentMethods` — он зависит от настроек вашего терминала и `restrictedPaymentMethods`. Стройте кнопки способов оплаты по нему:
...
| Параметр | Тип | Описание |
|---|---|---|
| `method` | `AltPayMethod` | `FasterPayments`, `SberPay`, `AlfaPay`, `MirPay`, `TPay`, `BNPL` |
| `opts.puid` | `String?` | Идентификатор попытки ([раздел 3](#puid--идентификатор-попытки-оплаты)). Настоятельно рекомендуем передавать |
| `opts.webview` | `Boolean` | `true`, если ссылку планируется открывать во встроенном WebView, а не во внешнем браузере/приложении банка |
...
// приложения банка на устройстве может не быть — тогда ACTION_VIEW бросает
// ActivityNotFoundException; держите наготове показ ссылки с копированием
try {
    startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(link)))
} catch (e: ActivityNotFoundException) {
    showCopyableLink(link)
}
...
### Прочие методы: AlfaPay, MirPay, T-Pay, BNPL
...
| `getPaymentMethod: метод X отсутствует в интенте` | Метод недоступен для терминала или запрещён `restrictedPaymentMethods` |
...
`YpConfig(baseUrl = "…", debugLogging = true)` включает лог всего, что делает SDK, в logcat под тегом `YpSdk`:

```bash
adb logcat -s YpSdk
```

В логе: HTTP-запросы и ответы целиком (заголовки, тела, код и длительность), а также шаги SDK — `createIntent`, `pay`, смены статуса интента, тики `waitForResult` с найденными транзакциями, ход 3-D Secure. Ответы длиннее лимита записи logcat разбиваются на пронумерованные части (`[1/15] …`).

> **Только для отладочных сборок.** Заголовки пишутся без маскирования, так что в logcat попадут значения из `YpConfig.headers`. Карточные данные в лог не попадают ни при каком значении флага: PAN, срок и CVV покидают устройство только внутри RSA-криптограммы. В продакшен-сборке оставляйте `debugLogging = false` (значение по умолчанию).
...
data class YpConfig(
    ...
    val debugLogging: Boolean = false,             // отладочный лог в logcat, см. раздел 14
)
```
...
CreateIntentRequest(
    // обязательные
    merchantCode: String,
    amount: Long,                  // минорные единицы, min 100
    currency: String,              // "RUB" | "USD" | "EUR"
    messageScheme: String,         // "SMS" | "DMS"
    // опциональные
    culture: String? = null,       // "ru-RU" | "en-US"
    ...
    restrictedPaymentMethods: List<String>? = null,
...
sealed class AltPayMethod(val type: String) {
    ...
    data object MirPay; data object TPay; data object BNPL
    companion object {
...
- [ ] `YpConfig.debugLogging` выключен — иначе весь обмен с API, включая заголовки, пишется в logcat.
